...
look_router = Blueprint('look_service', __name__,url_prefix='/api/v1/look')
look_service = LookService(LookRepository())

@look_router.route('/create', methods=['POST'])
def createLook():
    try:
...
@look_router.route('/update', methods=['POST'])
def updateLook():
    try:
        look_id = request.form.get("look_id")
