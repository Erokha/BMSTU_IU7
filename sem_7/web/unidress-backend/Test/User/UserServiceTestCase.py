...
def test_register_valid(repo, service):
    args = ['erokha', 'Nikita Erokhin', '123456', None]
    expect_return = {"login": "erokha", "name": "Nikita Erokhin", "password": "123456", "imageid": None}, False
    PyMock.setup(repo.register_user(*args)).returns(expect_return)

    response, code = service.register('erokha', 'Nikita Erokhin', '123456', None)
    assert response == '{"login": "erokha", "name": "Nikita Erokhin", "password": "123456", "imageid": null}'
    assert code == 200
...
def test_login(repo, service):
    args = ['erokha', '123456']
    expect_repo_response = {"login": "erokha", "name": "Nikita Erokhin", "password": "123456", "imageid": None}
