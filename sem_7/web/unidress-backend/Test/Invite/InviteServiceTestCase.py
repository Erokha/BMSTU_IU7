...
def test_who_invites_me_valid(repo, service):
    repo_return = [
        {
            'login_that_invites': 'sashazak',
            'wardrobe_name': 'gucci gang',
            'image_url': None
        },
    ]
    PyMock.setup(repo.get_invites_by_login('erokha')).returns(repo_return)

    response, code = service.who_invites_me('erokha')

    assert code == 200
    assert response == '[{"login_that_invites": "sashazak", "wardrobe_name": "gucci gang", "image_url": null}]'
