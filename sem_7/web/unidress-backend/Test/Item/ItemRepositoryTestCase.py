...
def test_correct_add_item():
    ...
    username = "erokha"
    user_repo.register_user(username, "Nikita Erokhin", "123456", None)
...
def test_correct_update_item():
    ...
    user_repo = UserRepository(DB_NAME, False)
    username = "sashazak"
    user_repo.register_user(username, "Sasha Zak", "123456", None)
    ...
    assert item_repo.get_item_by_id(responsItemAdd["item_id"]) == {'clothes_id': 1, 'clothes_name': 'new_name', 'type': 't-shirt', 'image_id': None, 'owner_login': 'sashazak', 'image_url': None}
